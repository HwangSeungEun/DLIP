...
{
	// read image  
	Mat img_gray = imread("../../Image/image.jpg", 0);   //graysacle
	Mat img = imread("../../Image/image.jpg", 1);
	...
	// For BGR image
	printf("%d\n", img.at<Vec3b>(0, 0)[0]); // Blue Pixel Value
	printf("%d\n", img.at<Vec3b>(0, 0)[1]); // Green Pixel Value
	printf("%d\n", img.at<Vec3b>(0, 0)[2]); // Red Pixel Value
	...
	{
		uchar* img_data = img.ptr<uchar>(v); // row of the image
	...
	int length = img_gray.rows * img_gray.cols * img_gray.channels(); // total data length
