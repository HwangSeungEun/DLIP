...
int main()
{
	...
	createTrackbar(trackbar_C,		window_name,  &threshold_C,		max_C,	   Threshold_Demo);
...
void Threshold_Demo(int, void*)	// default form of callback function for trackbar
{
	...
	*/
	if(threshold_type == 4)			adaptiveThreshold(src_gray, dst, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 3, threshold_C);
	else if(threshold_type == 5)	adaptiveThreshold(src_gray, dst, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 3, threshold_C);
	else							threshold(src_gray, dst, threshold_value, max_binary_value, threshold_type);
