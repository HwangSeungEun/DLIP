...
int main()
{
	...
	int contourA = 0;
	int face_idx = 0;
	findContours(dstSegment, contours, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
...
void selectMorph()//Morphology test
{
	...
	while (1)
	{
		...
		{
			morphologyEx(dstSegment, dstSegment, MORPH_CLOSE, element, Point(-1, -1), 5);
